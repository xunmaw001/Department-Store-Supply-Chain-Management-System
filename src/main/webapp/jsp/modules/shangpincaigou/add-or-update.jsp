		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑商品采购</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>商品采购管理</span></li>
								<li class="breadcrumb-item active"><span>编辑商品采购</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">商品采购信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>采购数量</label>
										<input id="caigoushuliang" name="caigoushuliang" class="form-control" onchange="calculation()">
									</div>
									<div class="form-group col-md-6">
										<label>价格</label>
									...
									<div class="form-group col-md-6">
										<label>总计</label>
										<input name="zongji" id="zongji" name="zongji" class="form-control" placeholder="总计">
									</div>
									<div class="form-group col-md-6">
										<label>产地</label>
									...
									<div class="form-group col-md-6">
										<label>采购时间</label>
										<div id="caigoushijianDate" class="input-append date form_datetime">
											<input id="caigoushijian-input" name="caigoushijian" size="20" type="text" readonly>
									...
									<div class="form-group col-md-6">
										<label>采购账号</label>
										<input name="caigouzhanghao" id="caigouzhanghao" name="caigouzhanghao" class="form-control" placeholder="采购账号">
									</div>
									<div class="form-group col-md-6">
										<label>采购人名</label>
										<input name="caigourenming" id="caigourenming" name="caigourenming" class="form-control" placeholder="采购人名">
									</div>
									<div class="form-group col-md-6">
										<label>采购说明</label>
										<input name="caigoushuoming" id="caigoushuoming" name="caigoushuoming" class="form-control" placeholder="采购说明">
									</div>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "shangpincaigou/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("shangpincaigou/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("shangpincaigou/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("shangpincaigou/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#caigoushijian-input").val(getCurDateTime());
			$("#caigoushijianDate").datetimepicker({ 
				autoclose: true,
		...
		function calculation(){
			...
			if($("#caigoushuliang").val() != null && $("#caigoushuliang").val() != ""){
				if(x == 0 && flag < 2){
					x = (x+1) * parseFloat($("#caigoushuliang").val());
					flag++;
				}else{
					x = x * parseFloat($("#caigoushuliang").val());
					flag++;
				}
			}
			...
			if(flag < 2){
				...
				$("#zongji").val(x.toFixed(2));
			}

		}		
		function calculationSE(colName){
			//单列求和接口
			http("shangpincaigou"+colName,"GET",{
			...
			function fill(){
				...
				http(accountTableName+'/session','GET',{},(res)=>{
					if(res.code == 0){
						...
						if(res.data !=null && res.data.caigouzhanghao != null && res.data.caigouzhanghao != ''){
							$('#caigouzhanghao').val(res.data.caigouzhanghao)
							$("#caigouzhanghao").attr('readonly','readonly');
						}
						if(res.data !=null && res.data.caigourenming != null && res.data.caigourenming != ''){
							$('#caigourenming').val(res.data.caigourenming)
							$("#caigourenming").attr('readonly','readonly');
						}
						$(validform());
					}
				});
		...
		function dataBind(){
			...
			$("#caigoushuliang").val(ruleForm.caigoushuliang);	
			$("#jiage").val(ruleForm.jiage);	
			$("#zongji").val(ruleForm.zongji);	
			$("#chandi").val(ruleForm.chandi);	
			$("#shengchanriqi").val(ruleForm.shengchanriqi);	
			$("#baozhiqi").val(ruleForm.baozhiqi);	
			$("#caigoushijian-input").val(ruleForm.caigoushijian);
			$("#caigouzhanghao").val(ruleForm.caigouzhanghao);	
			$("#caigourenming").val(ruleForm.caigourenming);	
			$("#caigoushuoming").val(ruleForm.caigoushuoming);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.caigoushuliang != null && res.data.caigoushuliang != ''){

						$("#caigoushuliang").val(res.data.caigoushuliang);
						$("#caigoushuliang").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.zongji != null && res.data.zongji != ''){

						$("#zongji").val(res.data.zongji);
						$("#zongji").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.caigoushijian != null && res.data.caigoushijian != ''){

						$("#caigoushijian-input").val(res.data.caigoushijian);
						$('#caigoushijianFileName').val(res.data.caigoushijian)
					}
					if(res.data != null && res.data != '' && res.data.caigouzhanghao != null && res.data.caigouzhanghao != ''){

						$("#caigouzhanghao").val(res.data.caigouzhanghao);
						$("#caigouzhanghao").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.caigourenming != null && res.data.caigourenming != ''){

						$("#caigourenming").val(res.data.caigourenming);
						$("#caigourenming").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.caigoushuoming != null && res.data.caigoushuoming != ''){

						$("#caigoushuoming").val(res.data.caigoushuoming);
						$("#caigoushuoming").attr('readonly','readonly');
					}
