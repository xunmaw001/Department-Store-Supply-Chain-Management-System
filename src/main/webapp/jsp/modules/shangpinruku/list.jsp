...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<label>入库时间</label>
									<input id="rukushijian_start" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
									至
									<input id="rukushijian_end" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('fuwuzhanghao')">服务账号<i id="fuwuzhanghaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('fuwurenming')">服务人名<i id="fuwurenmingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('rukushuoming')">入库说明<i id="rukushuomingIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			...
			if($('#rukushijian_start').val() != null && $('#rukushijian_start').val() != ''){
				searchForm.rukushijian_start = $('#rukushijian_start').val();
			}
			if($('#rukushijian_end').val() != null && $('#rukushijian_end').val() != ''){
				searchForm.rukushijian_end = $('#rukushijian_end').val();
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("shangpinruku/page","GET",{
				...
				rukushijian_start : searchForm.rukushijian_start,
				rukushijian_end : searchForm.rukushijian_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var fuwuzhanghaoCell  = document.createElement('td');

						 fuwuzhanghaoCell.innerHTML = item.fuwuzhanghao;

			row.appendChild(fuwuzhanghaoCell);
			var fuwurenmingCell  = document.createElement('td');

						 fuwurenmingCell.innerHTML = item.fuwurenming;

			row.appendChild(fuwurenmingCell);
			var rukushuomingCell  = document.createElement('td');

						 rukushuomingCell.innerHTML = item.rukushuoming;

			row.appendChild(rukushuomingCell);
